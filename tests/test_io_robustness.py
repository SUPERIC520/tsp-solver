...
from src.utils.data_io import save_hk_cache, load_hk_cache, load_tour
from src.core.seed_generation import rotate_tour

def test_hk_cache_shape_robustness(tmp_path, monkeypatch):
    ...
    loaded_lb, loaded_pi = load_hk_cache(sample_name)
    ...
    loaded_lb_1d, _ = load_hk_cache(sample_name)
    assert float(np.array(loaded_lb_1d).reshape(-1)[0]) == pytest.approx(lb_val)

def test_start_tour_rotation():
