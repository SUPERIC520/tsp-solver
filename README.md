...
This specific relaxation is evaluated algorithmically because the problem of finding a minimum weight 1-tree is solvable in $O(|E| \log |V|)$ operations. The calculation of $L(\pi)$ provides a bound for branch-and-bound processes. Furthermore, the final modified costs $c'_{ij}$ generated by the subgradient ascent procedure are utilized as edge weights to direct the $k$-opt search heuristic. Edges with values of $c'_{ij}$ approaching zero are included in the candidate sets evaluated during the sequential edge exchange operations.
