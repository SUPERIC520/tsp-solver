...
def run_trial(max_k, kicks, iters):
    # Running with --no_backbone if iters=1, otherwise backbone is essential for multi-iter
    backbone_flag = "" if iters > 1 else "--no_backbone"
    cmd = f"python src/scripts/full_scale_bench.py --n 500 --seeds 16 --kicks {kicks} --iters {iters} --max_opt {max_k} {backbone_flag}"
...
if __name__ == "__main__":
    ...
    for max_k, kicks, iters in configs:
        ...
        # Using 3 trials instead of 10 to fit in time and avoid excessive compute, 
        # as 16 seeds provide significant sample size naturally.
        for i in range(3): 
    ...
    with open("scratch/full_factorial_v2.csv", "w", newline="") as f:
